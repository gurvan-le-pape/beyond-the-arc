// src/backend/modules/matches/dto/matches.query.dto.ts
import { CompetitionLevel } from "@common/constants/competition-levels";
import { Category, Gender } from "@common/index";
import { ApiPropertyOptional } from "@nestjs/swagger";
import { Type } from "class-transformer";
import {
  IsEnum,
  IsNumber,
  IsOptional,
  IsString,
  Matches,
} from "class-validator";

/**
 * Query parameters for filtering and paginating matches.
 */
export class GetMatchesQueryDto {
  @ApiPropertyOptional({
    description: "Competition level (regional or departmental)",
    enum: [CompetitionLevel.REGIONAL, CompetitionLevel.DEPARTMENTAL],
    example: CompetitionLevel.REGIONAL,
  })
  @IsOptional()
  @IsEnum(CompetitionLevel, {
    message: `level must be either "${CompetitionLevel.REGIONAL}" or "${CompetitionLevel.DEPARTMENTAL}"`,
  })
  level?: CompetitionLevel;

  @ApiPropertyOptional({
    description: "Division number to filter matches",
    example: 1,
    type: Number,
  })
  @IsOptional()
  @Type(() => Number)
  @IsNumber()
  division?: number;

  @ApiPropertyOptional({
    description: "Committee ID (for departmental level)",
    example: 1,
    type: Number,
  })
  @IsOptional()
  @Type(() => Number)
  @IsNumber()
  committeeId?: number;

  @ApiPropertyOptional({
    description: "League ID (for regional level)",
    example: 1,
    type: Number,
  })
  @IsOptional()
  @Type(() => Number)
  @IsNumber()
  leagueId?: number;

  @ApiPropertyOptional({
    description: "Age category filter",
    example: "U18",
    type: String,
  })
  @IsOptional()
  @IsString({ message: "category must be a string" })
  category?: Category;

  @ApiPropertyOptional({
    description: "Gender filter",
    example: "M",
    type: String,
  })
  @IsOptional()
  @IsString({ message: "gender must be a string" })
  gender?: Gender;

  @ApiPropertyOptional({
    description: "Pool ID to filter matches",
    example: 1,
    type: Number,
  })
  @IsOptional()
  @Type(() => Number)
  @IsNumber()
  poolId?: number;

  @ApiPropertyOptional({
    description: "Championship ID to filter matches",
    example: 1,
    type: Number,
  })
  @IsOptional()
  @Type(() => Number)
  @IsNumber()
  championshipId?: number;

  @ApiPropertyOptional({
    description: "Matchday number",
    example: 1,
    type: Number,
  })
  @IsOptional()
  @Type(() => Number)
  @IsNumber()
  matchday?: number;

  @ApiPropertyOptional({
    description: "Date filter (YYYY-MM-DD format)",
    example: "2024-02-05",
    type: String,
  })
  @IsOptional()
  @IsString({ message: "date must be a string" })
  @Matches(/^\d{4}-\d{2}-\d{2}$/, {
    message: "date must be in YYYY-MM-DD format",
  })
  date?: string;

  @ApiPropertyOptional({
    description: "Search by club name (case-insensitive partial match)",
    example: "Paris",
    type: String,
  })
  @IsOptional()
  @IsString({ message: "search must be a string" })
  search?: string;

  @ApiPropertyOptional({
    description: "Page number for pagination",
    example: 1,
    type: Number,
    default: 1,
  })
  @IsOptional()
  @Type(() => Number)
  @IsNumber()
  page?: number;

  @ApiPropertyOptional({
    description: "Number of items per page (max 100)",
    example: 50,
    type: Number,
    default: 50,
  })
  @IsOptional()
  @Type(() => Number)
  @IsNumber()
  limit?: number;
}
